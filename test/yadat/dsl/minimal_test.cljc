...
(deftest resolve-find-spec-test
  (testing "relation"
    (let [find-spec (dsl/find-spec '[?id ?name])
          result (dsl/resolve-find-spec find-spec nil food-rows)]
...
(deftest resolve-clause-test
  (testing "pattern"
    (let [db (test-helper/recipe-db)
          ...
          [r1] (dsl/resolve-clause clause1 db [])
          [r2] (dsl/resolve-clause clause2 db [])]
      (is (= (:columns r1) '#{?name}))
  ...
  (testing "predicate"
    (let [relation (r/relation '#{?id ?name} food-rows)
          ...
          [relation] (dsl/resolve-clause clause nil [relation])]
  ...
  (testing "or"
    (let [db (test-helper/recipe-db)
          clause (dsl/where-clause '(or [?id :author/name "Adam"]
                                        [?id :author/name "Eve"]))
          [relation] (dsl/resolve-clause clause db [])]
  ...
  (testing "not"
    (let [db (test-helper/recipe-db)
          ...
          [relation] (dsl/resolve-clause clause db [relation])]
...
(deftest resolve-clauses-test
  (testing "pattern + predicate"
    (let [db (test-helper/recipe-db)
          ...
          relations (dsl/resolve-clause clause db [])
...
(deftest resolve-pull-element-test
  (testing "wildcard"
    (let [db (test-helper/recipe-db)
          ...
          entity (dsl/resolve-pull-element pull-element db {:db/id 10})]
      (is (= entity {:db/id 10
                     :author/name "Adam"
                     :author/gender "M"}))))
  ...
  (testing "map"
    (let [db (test-helper/recipe-db)
          ...
          entity (dsl/resolve-pull-element pull-element db {:db/id 42})]
      (is (= entity {:db/id 42
...
(let [db (test-helper/recipe-db)
  ...
  (dsl/resolve-pull-element pull-element db {:db/id 42})
  )

(deftest pull-test
  (testing "resolve lookup-reference"
    (let [db (test-helper/recipe-db)
          entities nil #_(dsl/pull db
                                     [:author/name "Adam"]
