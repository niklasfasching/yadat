...
* next
whether to rewrite shit to use clojure spec
clojure specs errors are horrible right now so they don't really replace real errors
but in theory they would
so switching to spec as parser + conform could be nice
...
- with clause
  works without it even though it shouldn't
  fix that
- db
  - db.fn
  - constraints (unique/value) during insert
- schema validation
...
- query
  ...
  - in + rules
- misc
  - reverse-ref, lookup-ref everywhere
  - consider some parsing strategy - https://github.com/youngnh/parsatron maybe can be adopted to lists
    also this https://gist.github.com/kachayev/b5887f66e2985a21a466
- tests
