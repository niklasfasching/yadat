need to lookup stuff from relations sometimes

problem with passing in non-relation variables
currently i resolve everything into a relation that's not a db
that leads to a problem when the value is actually used as a predicate..

-> more testing, rules
-> persistence? already have serialize / deserialize...

-> use it for a scraper, i.e. put data in there!

pullMap still unimplemented

as pull-pattern and fns and stuff can be bound via inputs have to carry those around to
- resolve-find-spec (FindPull, FindAggregate)
- resolve-clause (FunctionClause, PredicateClause)

-> rather than dbs the thingy could be sources!
-> sources are either raw values or dbs
-> if source not found for db take default source (get sources x *default-source*)
-> for fn resolution
