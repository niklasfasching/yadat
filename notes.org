* ToDo
- js interop
  will make using this ad-hoc in blogs possible
  tests http://blog.testdouble.com/posts/2015-10-12-configure-leiningen-for-clojurescript-and-clojure-testing

  -> js-api mirrors api but converts back and forward

- pull
- reverse ref in pull / elsewhere
- handle clause without var
- handle lookup-ref in pattern-clause (all clause types?)
- handle find aggregates
...
- workaround for resolve doesn't exist in cljs ([[https://stackoverflow.com/questions/12020576/resolve-function-throws-an-error-in-clojurescript-but-not-clojure/12020663#12020663][stackoverflow q]])
