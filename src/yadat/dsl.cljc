...
(defn update-cache
  ...
  A really naive cache that doesn't follow any strategy. A random entry is
  removed when the cache grows to big. Could be replaced by a LRU / LFU cache
  at some point but YAGNI."
...
(defrecord PullAttribute [a options])
(defrecord PullMap [a options pattern])
...
(defn parse-pull-element [form]
  (match [form]
    ...
    [(m :guard [map? #(= (count %) 1)])]
    (let [[a pattern] (first (seq m))
          [a o] (if (keyword? a) [a nil] [(first a) (apply hash-map (rest a))])]
      (->PullMap a o pattern))

    [([(a :guard keyword?) & (options :guard #(even? (count %)))] :seq)]
    (->PullAttribute a (apply hash-map options))
...
(defn parse-clause [form]
  (match [form]
    ...
    [[(src :guard util/src?) & pattern]]
    (->PatternClause src pattern)
